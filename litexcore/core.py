...
CPU_VARIANTS = {"standard": "CV32E41P",
    "HIDS_SIM":      "CV32E41P+HIDS+SIMULATION",
    "HIDS_SYNTH": "CV32E41P+HIDS+SYNTHESIS"}
...
GCC_FLAGS = {
    ...
    #                       imacfd
    "standard": "-march=rv32i2p0_mfd    -mabi=ilp32 ",
    "HIDS_SIM": "-march=rv32imc    -mabi=ilp32 ",
    "HIDS_SYNTH": "-march=rv32i2p0_mc   -mabi=ilp32 ",
   #"HIDS_SYNTH": "-march=rv32i2p0_mc    -mabi=ilp32 ",
}
...
class CV32E41P(CPU):
    ...
    def __init__(self, platform, variant="standard"):
        ...
        self.human_name        = CPU_VARIANTS.get(variant, "cv32e41p")
        self.reset             = Signal()
        ...
        # Define signal to monitor:
        self.GT_security_signal= Signal(32)

        self.csr_add           = Signal(12)
        self.csr_data           = Signal(32)
        self.alert           = Signal(2)
        self.analyze        = Signal()
        self.HPMcycles            = Signal(64)
        self.HPMinstr             = Signal(64) 
        self.HPMld_stall          = Signal(64)  
        self.HPMjmp_stall         = Signal(64)  
        self.HPMimiss             = Signal(64)
        self.HPMld                = Signal(64) 
        self.HPMst                 = Signal(64) 
        self.HPMjump                = Signal(64)
        self.HPMbranch             = Signal(64)
        self.HPMbranch_taken      = Signal(64)
        self.HPMcomp_instr          = Signal(64)
        self.HPMpip_stall           = Signal(64)   
        self.HPM_alert_counter           = Signal(32) 
        self.HPMrssi            = Signal(32)
        self.HPMsnr            = Signal(32)
        self.ewmaRSSIsig        = Signal(32)
        ...
        self.cpu_params = dict(
            ...
            # instanciate your signal:
            o_GT_security_signal = self.GT_security_signal,

            o_csr_add_test_i       = self.csr_add,
            o_csr_data_test_i       =self.csr_data,
            o_alert_o              = self.alert,
            o_analdone                 = self.analyze,
            o_hpm_RSSI = self.HPMrssi,
            o_hpm_SNR = self.HPMsnr,
            o_ewmaRSSI  = self.ewmaRSSIsig,
            o_hpm_cycles =self.HPMcycles,
            o_hpm_instr = self.HPMinstr,
            o_hpm_ld_stall =self.HPMld_stall,
            o_hpm_jpm_stall =self.HPMjmp_stall,
            o_hpm_imiss =self.HPMimiss,
            o_hpm_ld =self.HPMld,
            o_hpm_st =self.HPMst,
            o_hpm_jump = self.HPMjump,
            o_hpm_branch =self.HPMbranch,
            o_hpm_branch_taken = self.HPMbranch_taken,
            o_hpm_comp_instr = self.HPMcomp_instr,
            o_hpm_pip_stall = self.HPMpip_stall,
            o_hpm_alert_counter = self.HPM_alert_counter,
            # Controls.
            i_pulp_clock_en_i     = 1,
            i_scan_cg_en_i        = 0,
            i_mtvec_addr_i        = 0,
        ...
        # Add Verilog sources.
        #
        if variant in self.HIDS_SYNTH:
            add_manifest_sources(platform, 'cv32e41p_manifest_synth.flist')
        elif variant in self.HIDS_SIM:
            add_manifest_sources(platform, 'cv32e41p_manifest_sim.flist') 
        else : 
    ...
    def do_finalize(self):
        ...
        if self.variant in self.HIDS_SYNTH:
            self.specials += Instance("cv32e41p_core_synth", **self.cpu_params)
        elif self.variant in self.HIDS_SIM:
            self.specials += Instance("cv32e41p_core_sim", **self.cpu_params)
        else :
